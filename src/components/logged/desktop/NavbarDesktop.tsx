...
export default function NavbarDesktop({
  ...
  return (
    ...
    >
      ...
      <ul className={`flex flex-col mt-6 ${!isMenuOpen && "items-center"} gap-2 w-full px-2 overflow-y-auto max-h-[calc(100vh-250px)] pr-2`}>
        {role &&
      ...
      <div className="w-full mt-6 px-2" onClick={onOpenNotificationModal}>
        <div
          className={`flex items-center justify-between gap-2 p-2 rounded-md hover:bg-zinc-200 cursor-pointer ${isMenuOpen ? "pl-3 pr-4" : "justify-center"}`}
        >
          {isMenuOpen ? (
            <>
              ...
              <div className="bg-red-500 text-white text-xs font-bold w-6 h-6 rounded-full flex items-center justify-center">{unreadCount}</div>
            </>
          ) : (
      ...
      <div className={`${!isShortScreen ? "absolute bottom-4 w-[70%]" : "mt-4"}`}>
