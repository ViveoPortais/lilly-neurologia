...
export const Step2 = ({
  ...
  return (
    <>
        ...
        <Controller
          ...
          render={({ field }) => (
            <Input
              ...
              isLoading={role == "medico" ? isDoctorInfoLoading : undefined}
              onChange={(e) => {
      ...
      <div>
        <Controller name="cpf" control={control} render={({ field }) => maskedField("cpf", field.onChange, field.name, "CPF", true, () => {}, field.value)} />
      ...
      <div>
        ...
        <div className="flex flex-col ml-2 mt-1 space-y-1">
          {cellphoneError && <span className="text-sm text-red-500">{cellphoneError}</span>}
          {errors.telephoneNumber && <span className="text-sm text-red-500">{errors.telephoneNumber.message}</span>}
        </div>
      </div>
    </>
  );
