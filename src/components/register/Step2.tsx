...
export const Step2 = ({
  ...
  return (
    <>
      <div>
        <Input {...register("doctorName")} placeholder="Nome Completo" isLoading= {role == "medico" ? isDoctorInfoLoading : null} disabled={role == "medico"} />
      ...
      <div>
        <Input {...register("emailAddress")} placeholder="E-mail" isLoading= {role == "medico" ? isDoctorInfoLoading : null} />
      ...
      <div>
        <Controller
          ...
          render={({ field }) => {
            ...
            return maskedField("cellphone", handlePhoneChange, field.name, "Celular", false, () => {}, field.value);
          }}
        />
        {cellphoneError && <span className="ml-2 w-full text-xs text-red-400 mt-1">{cellphoneError}</span>}
        {errors.telephoneNumber && (
          <span className="ml-2 w-full text-xs text-red-400 mt-1">{errors.telephoneNumber.message}</span>
        )}
      </div>
    </>
  );
