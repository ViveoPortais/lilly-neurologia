...
import { allowedSpecialties, isValidPhoneNumber } from "@/helpers/helpers";
...
export default function MedicalSignUpModal({ role, onClose }: { role: string; onClose: () => void }) {
  ...
  const createListSpecialties = async (onlyAllowed = false) => {
    try {
      const response = await getListSpecialties();
      const filtered = onlyAllowed ? response.filter((item: IMedicalSpecialty) => allowedSpecialties.includes(item.name)) : response;
      const mapped = filtered.map((item: IMedicalSpecialty) => ({
    ...
    try {
      ...
      if (crm && ufcrm) {
        ...
        if (specialties.length === 0) {
          await createListSpecialties(true);
          return;
        }

        const isAllowed = specialties.some((spec: any) => allowedSpecialties.includes(spec));

        if (!isAllowed) {
          modal.showModal(
            {
              type: "warning",
              message: "Especialidade incompatível. O cadastro no Programa é restrito a especialidades médicas específicas. Em caso de dúvidas, consulte o Regulamento.",
            },
            () => {
              setValue("licenseState", "");
              setValue("doctorName", "");
        ...
        const matchedSpecialty = specialties.find((spec: any) => allowedSpecialties.includes(spec));
        if (matchedSpecialty) {
          const option = allowedSpecialties.find((opt) => opt.toUpperCase() === matchedSpecialty);
          if (option) {
            setValue("medicalSpecialty", option);
          }
        }
      }
    } catch (error) {
