...
const Page = () => {
 ...
 const programSlug = auth?.programSlug as ProgramSlug | undefined;
 const role = auth?.role as AppRole | undefined;
 const userRoutes = routesByProgram?.[programSlug as ProgramSlug]?.[role as AppRole] ?? [];
...
useEffect(() => {
  if (!auth?.programSlug || !auth?.role || !auth?.programsCode?.length) return;
...
const handleCardClick = async (path: string, openModal?: boolean) => {
  if (!isConsentOk()) {
    const ok = await ensureConsentOk(() => getConsentStatus());
    if (!ok) return; 
  }

  const fullRoute = buildDashboardHref(programSlug!, path);
  
  if (openModal) {
    setPendingRoute(fullRoute);
