 ...
 return (
  <form onSubmit={handleSubmit(onSubmit)} className="w-full">
   <Controller
    ...
    control={control}
    render={({ field }) => (
     <CustomSelect name="role" label="Qual seu perfil?" options={profileOptions} value={field.value} onChange={field.onChange} />
    )}
   />
   ...
   {role === "medico" && (
    <div className="flex flex-col gap-4 mt-4">
     ...
     <Controller
      ...
      control={control}
      render={({ field }) => (
       <CustomSelect name="licenseState" label="UF do CRM" options={UFlist} value={field.value} onChange={field.onChange} />
      )}
     />
     {(errors as any).licenseState && <p className="text-red-500 text-sm">{(errors as any).licenseState.message}</p>}
